...
export default function One({ data }: { data: IOne}) {
    return (
        <>
            {data.map((item, index) => (
                <div key={index}>
                    <div className="w-full flex flex-row mt-8 shadow-xl/5 shadow-black rounded-xl border border-gray-200 h-[293px]">
                        <div className="w-1/2 pl-8 pr-4 mt-8">
                            ...
                            <div className="group mt-24 border border-black w-28 flex justify-center items-center py-1 rounded-full hover:bg-gray-900 hover:cursor-pointer transition-colors">
                                <a href={item.link} target="_blank" className="flex gap-2 items-center">
                                    <span className="text-sm font-medium text-black group-hover:text-white">Visit site</span>
